...
export function RatingPopup() {
  ...
  const checkServerRatingStatus = async (token: string): Promise<boolean> => {
    try {
      const response = await fetch('https://localhost:7234/api/WebsiteRatings/hasRated', {
        method: 'GET',
        headers: {
  ...
  const checkAuthAndRatingStatus = async () => {
    ...
    try {
      ...
      const serverHasRated = localHasRated ? true : await checkServerRatingStatus(token)
